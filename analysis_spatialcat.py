...
def analyze_model_from_file(results,lesion = False, simulation = True):
    ...
    if lesion:
        ...
        lesion_results = lesion_weights_spatialcat(trial_info, h_stacked, weights)
    ...
    if simulation:
        ...
        simulation_results = simulate_network_spatialcat(trial_info_whiteonly, h_stacked, weights)
...
##NEW NEW
def lesion_weights_spatialcat(trial_info, h, weights):
    ...
    for n1 in range(3):
        for n2 in range(N):
            print('Neuron {:3d}'.format(n1) + '| Neuron {:3d}'.format(n2))
            ...
            # simulate network
            y_hat, hidden_state_hist,_,_ = run_model(x, y, hidden_init, weights_new)
...
def simulate_network_spatialcat(trial_info, h, network_weights):
    ...
    train_mask = trial_info['train_mask']
    hidden_init = h[:,test_onset-1,:]
    y_hat, hs_hist_wonly, _, _ = run_model(x, y, hidden_init, network_weights)
    accuracy = get_perf(y, y_hat, train_mask)

    h_stacked = np.stack(hs_hist_wonly, axis=1)
    trial_time = np.arange(0,h_stacked.shape[1]*par['dt'], par['dt'])
    simulation_results = {
        ...
        'trial_time': trial_time}
    simulation_results['hs_hist_wonly'] = hs_hist_wonly
    simulation_results['trial_info'] = trial_info
...
def run_model(x, y, hidden_init, weights, syn_x_init = None, syn_u_init = None, suppress_activity = None):
